...
// واعتماد GetPayIn بالكامل (بعد ما كان Paymob). كل كورس/خطة اشتراك سعره
// مخزّن بشكل منفصل لكل عملة من التلاتة المدعومة (EGP/USD/EUR) في
// الداتابيز — مفيش تحويل عملة وقت الدفع نفسه، ولا ربط ديناميكي بين
// الأسعار بعد الحفظ. العملة اللي المستخدم بيدفع بيها بتتحدد تلقائيًا حسب
// لغة الموقع الحالية (مش حسب موقعه الجغرافي أو أي حاجة تانية):
//
...
//
// 🆕 ملاحظة عن GetPayIn: على عكس Paymob (اللي كان integration واحد مربوط
// بعملة واحدة بس)، GetPayIn مش مربوط بعملة معيّنة في التكامل نفسه — نفس
// public_token/hash_token شغالين لأي عملة العميل حاططها فعليًا مفعّلة في
// حسابه عند GetPayIn. لو عملة معيّنة مش مفعّلة في الحساب، GetPayIn هترجع
// خطأ مباشر على نداء إنشاء الـ invoice — مش مشكلة في الكود هنا.
