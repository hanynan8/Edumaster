  ...
  return (
    <div className="mt-4 pt-4 border-t border-gray-100">
      <h4 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
        <MessageCircle size={15} className="text-[#0f2d57]" /> {t.title}
      </h4>
      ...
      <form onSubmit={submitQuestion} className="flex items-center gap-2 mb-4">
        <input
          ...
          maxLength={2000}
          className="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-[#0f2d57]"
        />
        <button
          type="submit"
          disabled={posting || !newQuestion.trim()}
          className="shrink-0 bg-[#0f2d57] text-white p-2 rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity"
          aria-label={t.send}
        >
          <Send size={15} />
        </button>
              ...
              {replyOpenFor === c.id ? (
                <div className="mt-2 ms-4 flex items-center gap-2">
                  <input
                    ...
                    maxLength={2000}
                    className="flex-1 text-xs border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:border-[#0f2d57]"
                  />
                  <button onClick={() => submitReply(c.id)} className="text-xs font-semibold text-[#0f2d57] shrink-0">
                    {t.send}
                  </button>
                </div>
              ) : (
                <button
                  ...
                  className="mt-2 ms-4 text-[11px] font-semibold text-gray-400 hover:text-[#0f2d57] transition-colors"
                >
                  {t.reply}
                </button>
              )}
