              ...
              return (
                <div
                  ...
                  className={`relative bg-white rounded-2xl border p-6 flex flex-col ${
                    isCurrent ? "border-[#003A91] ring-2 ring-[#003A91]/20" : "border-gray-100"
                  }`}
                >
                  {isCurrent && (
                    <span className="absolute -top-3 start-6 text-[10px] font-bold bg-[#003A91] text-white px-3 py-1 rounded-full">
                      {t.subscribed}
                    </span>
                  )}
                    ...
                    <ul className="space-y-2 mb-6 flex-1">
                      {plan.features.map((f, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-gray-600">
                          <Check size={15} className="text-[#003A91] shrink-0 mt-0.5" /> {f}
                        </li>
                      ))}
                    </ul>
                  )}
