...
import { ArrowRight, Loader, Users, CheckCircle2, XCircle, Minus } from "lucide-react";
...
function AssignmentCell({ grade, maxScore }) {
  if (!grade.submitted) return <Minus size={14} className="text-gray-300 mx-auto" />;
  if (grade.status !== "graded") {
    return <span className="text-[11px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">قيد التصحيح</span>;
  }
  return (
...
export default function CoursePerformancePage({ params }) {
  ...
  useEffect(() => {
    fetch(`/api/courses/${id}/performance`)
      ...
      .catch(() => setError("تعذّر تحميل بيانات الأداء"));
  }, [id]);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-gray-500 text-xs">
                <th className="text-start px-4 py-3 font-semibold sticky start-0 bg-gray-50">الطالب</th>
                <th className="text-start px-4 py-3 font-semibold">نسبة الإكمال</th>
                {data.quizzes.map((q) => (
                  ...
                  {s.assignmentGrades.map((g) => {
                    ...
                    return (
                      <td key={g.assignmentId} className="px-4 py-3 text-center">
                        <AssignmentCell grade={g} maxScore={assignment?.maxScore || 100} />
                      </td>
                    );
                  })}
                </tr>
