...
const ALLOWED_KINDS = {
  image: { subfolder: "images", allowedMime: (m) => m.startsWith("image/") },
  pdf: { subfolder: "files", allowedMime: (m) => m === "application/pdf" },
};
...
export async function POST(request) {
  try {
    ...
    // 🔒 المسار مبني من بيانات معروفة عندنا بس (id المستخدم + نوع الملف)،
    // مش من أي path جاي من الـ client — كده مفيش path traversal ولا مدرس
    // يقدر يكتب في فولدر مدرس تاني.
    const path = `edumaster/teachers/${session.user.id}/${subfolder}/${safeFileName(file.name)}`;
