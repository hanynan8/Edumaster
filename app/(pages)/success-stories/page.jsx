        ...
        <div className="max-w-7xl mx-auto">
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-semibold tracking-tight max-w-2xl mb-4 sm:mb-5 animate-fadein-up leading-[1.05]">
            {(() => {
              ...
              return (<><span className="text-[#0a0a0a]">{words.slice(0, 2).join(" ")}</span><br /><span className="text-[#003A91]">{words.slice(2).join(" ")}</span></>);
            })()}
          </h1>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-px bg-gray-200 rounded-2xl overflow-hidden">
        {data.stats.items.map((s, i) => (
          <div key={i} className={`bg-white px-5 sm:px-8 py-7 sm:py-10 flex flex-col gap-1 transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
            ...
            <div className="w-4 sm:w-5 h-0.5 bg-[#003A91] mt-1.5 sm:mt-2" />
          </div>
        ))}
      </div>
    </section>
    ...
    <section ref={ref} className="py-16 sm:py-20 md:py-24 px-5 sm:px-8 md:px-6 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {merged.map((item, i) => (
            <div key={item.id}
              className={`bg-[#f7f7f7] rounded-2xl p-5 sm:p-7 flex flex-col gap-4 sm:gap-5 border border-gray-100 hover:border-[#003A91]/20 hover:shadow-lg transition-all duration-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              style={{ transitionDelay: `${i * 70}ms` }}>
              <div className="text-[#003A91]/15"><Quote size={28} /></div>
              ...
              <span className="self-start text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full text-white"
                style={{ background: TAG_COLORS[item.type] ?? "#003A91" }}>{item.tag}</span>
...
//       <div className="absolute top-0 inset-x-0 h-[3px] bg-[#003A91]" />
...
//           <Link href="/contact"
//             className="inline-flex items-center gap-2 bg-[#003A91] text-white font-bold px-7 sm:px-8 py-3.5 sm:py-4 rounded-lg text-sm sm:text-base hover:bg-[#a50d24] transition-colors shadow-lg">
