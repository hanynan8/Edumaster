...
export async function middleware(request) {
  ...
  // Google Fonts، و connect-src لـ GetPayIn Payment Integration API (السيرفر
  // بينادي GetPayIn من الباك اند مش من المتصفح فعليًا — الدفع بيتم عن طريق
  // redirect كامل بـ window.location.href لصفحة الدفع المستضافة عند
  // GetPayIn، مش SDK/iframe مضمّن جوه صفحتنا، فمفيش داعي لإضافة نطاق
  // getpayin.com في script-src؛ frame-src مضاف لـ pay.getpayin.com للاحتياط
  // بس (لو حصل تغيير مستقبلي لتضمين iframe بدل redirect كامل).
  //
  ...
  // placehold, Google Fonts, GetPayIn Payment Integration API), فمفيش سبب تفضل Report-Only —
  ...
  const cspDirectives = [
    ...
    "frame-src 'self' https://iframe.mediadelivery.net https://pay.getpayin.com https://*.daily.co",
    "connect-src 'self' https://*.b-cdn.net https://video.bunnycdn.com https://pay.getpayin.com https://*.daily.co wss://*.daily.co",
    "object-src 'none'",
    "base-uri 'self'",
    "form-action 'self'",
