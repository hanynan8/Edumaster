    ...
    return (
      <div className="border-b border-gray-100 last:border-0">
        {canOpenQuiz ? (
          ...
          >
            <Icon size={17} className="text-[#003A91] shrink-0" />
            ...
            <span className="text-xs font-semibold text-[#003A91] shrink-0">{t.takeQuiz}</span>
          </Link>
        ) : (
        ...
        <div className="px-4 sm:px-5 pb-4">
          ...
          {lesson.type === "pdf" && lesson.fileUrl && (
            <a href={lesson.fileUrl} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-sm font-semibold text-[#003A91] hover:underline">
  ...
  if (notFound) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-24 text-center">
        ...
        <Link href="/courses" className="text-[#003A91] font-semibold hover:underline">{t.back}</Link>
      </div>
    );
  }
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
          ...
          <div className="grid lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              {loc.categoryName && (
                <span className="inline-block text-[11px] font-bold uppercase tracking-widest text-[#003A91] bg-white/10 px-3 py-1 rounded-full mb-4">
                  {loc.categoryName}
                </span>
              )}
              ...
              <div className="p-5">
                ...
                {isOwner ? (
                  ...
                  <button
                    ...
                    className="w-full bg-[#003A91] text-white font-bold py-3 rounded-xl hover:opacity-90 transition-opacity"
                  >
                    {t.login}
                  </button>
                ) : (
                  <button
                    ...
                    className="w-full bg-[#003A91] text-white font-bold py-3 rounded-xl hover:opacity-90 transition-opacity disabled:opacity-60"
                  >
                    {enrolling
          ...
          <div className="space-y-6">
            ...
            {isEnrolled && !isOwner && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-black text-[#003A91]">
            ...
            {isEnrolled && session?.user?.role === "student" && (quizzes.length > 0 || assignments.length > 0) && (
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                      ...
                      {quizzes.map((q) => (
                        <li key={q.id}>
                          <Link
                            ...
                            className="flex items-center gap-2 text-sm text-gray-600 hover:text-[#003A91] transition-colors"
                          >
                            <HelpCircle size={14} className="text-[#003A91] shrink-0" />
                      ...
                      {assignments.map((a) => (
                        <li key={a.id}>
                          <Link
                            ...
                            className="flex items-center gap-2 text-sm text-gray-600 hover:text-[#003A91] transition-colors"
                          >
                            <FileText size={14} className="text-[#003A91] shrink-0" />
                ...
                <Link
                  href="/student/grades"
                  className="block text-center text-xs font-semibold text-[#003A91] hover:underline mt-4 pt-4 border-t border-gray-100"
                >
                  {t.myGradesLink}
                </Link>
              </div>
                ...
                <ul className="space-y-2">
                  {loc.outcomes.map((o, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-gray-600">
                      <CheckCircle2 size={15} className="text-[#003A91] shrink-0 mt-0.5" /> {o}
                    </li>
                  ))}
                </ul>
              </div>
